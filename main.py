...
fantasy-bball-ranker CLI
  ...
  python main.py ingest                         Pull stats from ESPN for 6 seasons -> SQLite
...
def cmd_status() -> None:
    ...
    rankings_file = BASE_DIR / "outputs" / "rankings.xlsx"
    last_run      = rankings_file.stat().st_mtime if rankings_file.exists() else None
    ...
    if last_run:
        ...
        print(f"Last rankings:     {datetime.fromtimestamp(last_run).strftime('%Y-%m-%d %H:%M')}")
    else:
        print(f"Last rankings:     --")
